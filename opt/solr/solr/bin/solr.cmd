...
IF "%SOLR_INCLUDE%"=="" set "SOLR_INCLUDE=%SOLR_TIP%\..\solr.in.cmd"
...
:start_solr
IF "%SOLR_HOME%"=="" set "SOLR_HOME=%SOLR_TIP%\..\solrhome"
...
IF "%SCRIPT_CMD%"=="stop" (
  IF "%SOLR_PORT%"=="" (
    IF "%STOP_ALL%"=="1" (
      set found_it=0
      for /f "usebackq" %%i in (`dir /b "%SOLR_TIP%\bin" ^| C:\Windows\System32\findstr.exe /i "^solr-.*\.port$"`) do (
        set SOME_SOLR_PORT=
        For /F "delims=" %%J In ('type "%SOLR_TIP%\bin\%%i"') do set SOME_SOLR_PORT=%%~J
        if NOT "!SOME_SOLR_PORT!"=="" (
          for /f "tokens=2,5" %%j in ('C:\Windows\System32\netstat.exe -aon ^| C:\Windows\System32\find.exe "TCP " ^| C:\Windows\System32\find.exe ":0 " ^| C:\Windows\System32\find.exe ":!SOME_SOLR_PORT! "') do (
            ...
            IF NOT "%%k"=="0" (
              IF "%%j"=="%SOLR_JETTY_HOST%:!SOME_SOLR_PORT!" (
                ...
                For /f "tokens=2,5" %%M in ('C:\Windows\System32\netstat.exe -nao ^| C:\Windows\System32\find.exe "TCP " ^| C:\Windows\System32\find.exe ":0 " ^| C:\Windows\System32\find.exe ":!SOME_SOLR_PORT! "') do (
                  IF "%%N"=="%%k" (
    ...
    set found_it=0
    For /f "tokens=2,5" %%M in ('C:\Windows\System32\netstat.exe -nao ^| C:\Windows\System32\find.exe "TCP " ^| C:\Windows\System32\find.exe ":0 " ^| C:\Windows\System32\find.exe ":%SOLR_PORT% "') do (
      IF NOT "%%N"=="0" (
...
IF "%SCRIPT_CMD%"=="start" (
  REM see if Solr is already running using C:\Windows\System32\netstat.exe
  For /f "tokens=2,5" %%j in ('C:\Windows\System32\netstat.exe -aon ^| C:\Windows\System32\find.exe "TCP " ^| C:\Windows\System32\find.exe ":0 " ^| C:\Windows\System32\find.exe ":%SOLR_PORT% "') do (
    IF NOT "%%k"=="0" (
      IF "%%j"=="%SOLR_JETTY_HOST%:%SOLR_PORT%" (
...
C:\Windows\System32\findstr.exe /i /C:" %SOLR_LOGS_DIR% " "%TEMP%\solr-pattern.txt" 1>nul
if %ERRORLEVEL% == 0 (
...
REM  "%JAVA%" %SOLR_SSL_OPTS% %AUTHC_OPTS% %SOLR_ZK_CREDS_AND_ACLS% -Dsolr.install.dir="%SOLR_TIP%" ^
REM    -Dlog4j.configuration="file:%DEFAULT_SERVER_DIR%\scripts\cloud-scripts\log4j.properties" ^
REM    -classpath "%DEFAULT_SERVER_DIR%\solr-webapp\webapp\WEB-INF\lib\*;%DEFAULT_SERVER_DIR%\lib\ext\*" ^
REM    org.apache.solr.util.SolrCLI status -maxWaitSecs 30 -solr !SOLR_URL_SCHEME!://%SOLR_TOOL_HOST%:%SOLR_PORT%/solr
)
...
if "!CREATE_PORT!"=="" (
  set "SCRIPT_ERROR=Could not C:\Windows\System32\find.exe a running Solr instance on this host! Please use the -p option to specify the port."
  goto err
)
...
FOR /f "usebackq tokens=3" %%a IN (`^""%JAVA%" -version 2^>^&1 ^| C:\Windows\System32\findstr.exe "version"^"`) do (
...
:resolve_java_vendor
"%JAVA%" -version 2>&1 | C:\Windows\System32\findstr.exe /i "IBM J9" > nul

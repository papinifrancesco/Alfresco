...
#   users_rest.txt            \Uffffffff sorted distinct usernames (lowercased)
#   users_rest_by_month.txt   \Uffffffff YYYY-MM, distinct active users that month
#   entries_rest.tsv          \Uffffffff raw paginated dump (id, time_ms, user) for audit
#   stdout                    \Uffffffff total count and time-span
...
# ALF_INSECURE=1 disables TLS cert verification (for internally-signed
# Alfresco servers). Prefer ALF_CACERT pointing at the org CA bundle.
curl_tls=()
if [ -n "${ALF_CACERT:-}" ]; then
    curl_tls+=(--cacert "$ALF_CACERT")
elif [ "${ALF_INSECURE:-0}" = "1" ]; then
    curl_tls+=(-k)
fi
...
while :; do
    page=$(curl -kfsSu "$ALF_USER:$ALF_PASS" "${curl_tls[@]}" \
        --get \
